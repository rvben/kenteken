...
/// Render a day count as a human phrase: `in 1 year 4 months`, `3 months ago`.
///
/// Deliberately coarse. The exact day is already printed next to it; this phrase
/// answers "is this a problem" at a glance. Written out in full rather than
/// abbreviated, because `1y 4m` reads as a part number in a column of dates.
pub fn humanize_offset(days: i64) -> String {
    if days == 0 {
...
pub fn humanize_span(days: i64) -> String {
    ...
    if magnitude < 45 {
        ...
        match (magnitude as f64 / 30.44).round().max(1.0) as u64 {
            12 => units(1, "year"),
            m => units(m, "month"),
        }
    } else {
        let years = magnitude / 365;
        let months = ((magnitude % 365) as f64 / 30.44).round() as u64;
        match months {
            0 => units(years, "year"),
...
fn units(n: u64, noun: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn humanizes_future_and_past_symmetrically() {
        assert_eq!(humanize_offset(0), "today");
        assert_eq!(humanize_offset(1), "in 1 day");
        assert_eq!(humanize_offset(-1), "1 day ago");
        assert_eq!(humanize_offset(30), "in 30 days");
    ...
    #[test]
    fn humanizes_multi_year_offsets() {
        assert_eq!(humanize_offset(494), "in 1 year 4 months");
        assert_eq!(humanize_offset(-494), "1 year 4 months ago");
        assert_eq!(humanize_offset(365), "in 1 year");
        assert_eq!(humanize_offset(730), "in 2 years");
        assert_eq!(humanize_offset(396), "in 1 year 1 month");
    }
    ...
    fn rounding_never_produces_twelve_months() {
        ...
        for days in 350..800 {
            let phrase = humanize_offset(days);
            assert!(!phrase.contains("12 month"), "day {days} rendered {phrase}");
        }
    }

    #[test]
    fn nothing_is_ever_pluralised_as_one_of_something() {
        // A phrase reading "in 1 years" is the tell that a unit was formatted
        // without checking its count.
        for days in -900i64..900 {
            let phrase = humanize_offset(days);
            // Compared word by word: "11 months" ends in the same letters as
            // "1 months" and is perfectly correct.
            let words: Vec<&str> = phrase.split(' ').collect();
            for pair in words.windows(2) {
                assert!(
                    pair[0] != "1" || !pair[1].ends_with('s'),
    ...
    fn month_phrasing_never_says_zero_months() {
        for days in 45..365 {
            let phrase = humanize_offset(days);
            assert!(!phrase.starts_with("in 0"), "day {days} rendered {phrase}");
        }
    }
}
